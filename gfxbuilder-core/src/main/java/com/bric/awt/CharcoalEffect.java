				...
				if(distance+pendingGap>=orig) {
					...
					if(addCrack) {
						for(int mult = -1; mult<=1; mult+=2) { //try both the clockwise and the counterclockwise side
							...
							while(s.contains(center.getX()+width*Math.cos(angle+mult*Math.PI/2),
									center.getY()+width*Math.sin(angle+mult*Math.PI/2)) && width<maxDepth) {
								width++;
							}
							...
							if(width>1) {
								...
								if(k>1) k = 1; //cap at these values
								if(k<.4f) k = .4f;
								depth = width*k*(.5f+.5f*size)*(.25f+.75f*random.nextFloat());
								
								crackWidth = depth*depth/150;
								if(crackWidth<1f) crackWidth = 1f;
								if(crackWidth>2) crackWidth = 2;
								...
								if(distance+pendingGap-crackWidth/2>0 && distance+pendingGap+crackWidth/2<orig) {
									...
									writer.lineTo(
											(float)(center.getX()+depth*Math.cos(angle+mult*Math.PI/2)),
											(float)(center.getY()+depth*Math.sin(angle+mult*Math.PI/2))
									);
